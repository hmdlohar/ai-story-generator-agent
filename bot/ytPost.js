...
function findChromeBinary() {
  ...
  for (const name of CHROME_CANDIDATES) {
    const path = process.env.PATH.split(':').map((d) => `${d}/${name}`).find(existsSync);
    if (path) return path;
  }
  return null;
}
...
function findChromeMainPid(chromeBin) {
  const proc = spawn('pgrep', ['-f', `${chromeBin} --`], { stdio: ['ignore', 'pipe', 'pipe'] });
...
async function connectWithAutoLaunch(cdpUrl) {
  ...
  if (!chromeBin) {
    throw new Error(`No Chrome binary found. Start Chrome with --remote-debugging-port=${port} manually, or install google-chrome-unstable.`);
  }
  ...
  if (existingPid) {
    console.log(`Chrome is running without CDP (pid ${existingPid}). Quitting it so we can relaunch with the debugging port...`);
    try {
    ...
    if (!(await waitForPidExit(existingPid, 15000))) {
      throw new Error(`Could not quit the running Chrome (pid ${existingPid}). Close it manually and retry.`);
    }
    await new Promise((r) => setTimeout(r, 1500));
  }

  console.log(`Launching ${chromeBin} with remote debugging on port ${port}...`);
  ...
  if (!(await waitForCdp(url, 45000))) {
    throw new Error(`Chrome launched but CDP did not come up on ${url} within 45s`);
  }
...
async function setPaperInputValue(page, inputSelector, value) {
  const ok = await page.evaluate((sel, val) => {
    ...
    const input = (
      candidates.find((i) => scope && scope.contains(i) && i.offsetParent !== null) ||
...
async function typeScheduleDate(page, date) {
  ...
  const dayFirst = date.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }); // "2 Aug 2026"
  // Wait for the calendar dialog to actually open (its input becomes visible).
  await page.waitForFunction(
    () => [...document.querySelectorAll('ytcp-date-picker [aria-label="Enter date"] input')].some((i) => i.offsetParent !== null),
    { timeout: 5000 }
  ).catch(() => {});
  await setPaperInputValue(page, 'ytcp-date-picker [aria-label="Enter date"] input', dayFirst);
...
async function typeScheduleTime(page, date) {
  ...
  await setPaperInputValue(page, 'ytcp-datetime-picker #time-of-day-container input', `${hh}:${mm}`);
  ...
  await page.waitForFunction(
    (label) => {
      const el = [...document.querySelectorAll('button')].find(
        (b) => (b.getAttribute('aria-label') || '').trim() === label && b.offsetParent !== null
      );
      if (!el) return false;
      el.focus();
  ...
  await page.waitForFunction(
    (needle) => {
      const el = [...document.querySelectorAll('tp-yt-paper-item')].find(
        (item) => item.offsetParent !== null && item.getAttribute('test-id') === needle
      );
      if (!el) return false;
      el.focus();
      return document.activeElement === el;
...
async function dismissOverlays(page) {
  await page.evaluate(() => {
    const dialog = [...document.querySelectorAll('ytcp-dialog, ytcp-banner')].find(
      (el) => el.offsetParent !== null
    );
    if (dialog) {
      const btn = [...dialog.querySelectorAll('button')].find((b) =>
        /dismiss|close|get started/i.test(b.getAttribute('aria-label') || b.innerText || '')
      );
      if (btn) btn.click();
    }
    ...
    for (const b of [...document.querySelectorAll('tp-yt-iron-overlay-backdrop.opened')]) {
      b.remove();
    }
  });
  await new Promise((r) => setTimeout(r, 800));
...
export async function uploadProject({
  ...
  try {
    ...
    if (dialogOpen) {
      ...
      for (let attempt = 0; attempt < 5 && !menuOpened; attempt++) {
        ...
        menuOpened = await page.evaluate(() =>
          [...document.querySelectorAll('tp-yt-paper-item')].some(
            (item) => item.offsetParent !== null && (item.getAttribute('test-id') || '') === 'upload'
          )
        );
      }
      if (!menuOpened) throw new Error('Could not open the Create menu (5 attempts)');
      ...
      await page.waitForFunction(
        () => {
          const dlg = document.querySelector('ytcp-uploads-dialog');
          if (!dlg) return false;
          return !!dlg.querySelector('ytcp-social-suggestions-textbox#title-textarea');
        },
        { timeout: 600000 }
      );
    }
    ...
    if (!dialogOpen) {
      ...
      const titleField = await page.waitForSelector('ytcp-social-suggestions-textbox#title-textarea #textbox');
      ...
      const descField = await page.waitForSelector('ytcp-social-suggestions-textbox#description-textarea #textbox');
      ...
      const tagsField = await page.waitForSelector('#tags-input, input[aria-label="Tags"]', { timeout: 15000 });
    ...
    try {
      await page.waitForFunction(
        () => {
          const dlg = document.querySelector('ytcp-uploads-dialog');
          if (!dlg || dlg.offsetParent === null) return false;
          if (/choose when to publish|made for kids/i.test(dlg.innerText || '')) return true;
          return !!dlg.querySelector('ytcp-video-visibility-select #second-container');
        },
        { timeout: 15000 }
      );
    } catch {
      throw new Error('Reached the Visibility screen but the form did not show');
    }
    ...
    await page.evaluate(() => {
      const c2 = document.querySelector('ytcp-uploads-dialog ytcp-video-visibility-select #second-container');
      if (!c2) return false;
      c2.click();
      return true;
    });
    ...
    await page.evaluate(() => {
      ...
      const btn = [...dlg.querySelectorAll('button, ytcp-icon-button')].find(
        (b) => (b.getAttribute('aria-label') || '').trim() === 'Click to expand' && b.offsetParent !== null
      );
      if (btn) btn.click();
    });
    await new Promise((r) => setTimeout(r, 1200));
    ...
    const confirmed = await page.waitForFunction(
      () => {
        ...
        return [...dlg.querySelectorAll('a')].some(
          (a) => /^https:\/\/youtube\.com\//.test(a.href || '') || /^https:\/\/www\.youtube\.com\//.test(a.href || '')
        );
      },
      { timeout: 15000 }
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      await page.evaluate(() => {
        const warn = [...document.querySelectorAll('ytcp-dialog, tp-yt-paper-dialog')].find(
          (d) => d.offsetParent !== null && /still checking|come back/i.test(d.innerText || '')
        );
        const btn = warn
          ? [...warn.querySelectorAll('button')].find((b) => b.offsetParent !== null && /got it/i.test(b.innerText || ''))
          : null;
        if (btn) btn.click();
      });
      await new Promise((r) => setTimeout(r, 1200));
    ...
    if (stillOpen) {
      ...
      await new Promise((r) => setTimeout(r, 2500));
      await page.evaluate(() => {
        for (const d of [...document.querySelectorAll('ytcp-dialog, tp-yt-paper-dialog')]) {
          if (d.offsetParent !== null && !(d.innerText || '').trim()) d.remove();
        }
      });
    }
...
if (isMain) {
  ...
  const scheduleRaw = scheduleIdx >= 0 ? process.argv[scheduleIdx + 1] : process.argv[3] || null;

  if (!projectRef || !scheduleRaw) {
    console.error('Usage: node bot/ytPost.js <project-name> --schedule "YYYY-MM-DD HH:MM"');
    console.error('  e.g. node bot/ytPost.js p20 --schedule "2026-08-05 18:30"');
    process.exit(1);
  }

  const result = await uploadProject({ projectRef, scheduleDate: new Date(scheduleRaw) });
